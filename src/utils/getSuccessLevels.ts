...
export default (
  ...
  const isFumble =
    finalDieResult >= fumbleMinLevel && finalDieResult <= fumbleMaxLevel;
  const isRegularFailure = !isFumble && isFailure;

  const isCriticalSuccess = finalDieResult === critSuccessLevel;
  const isExtremeSuccess =
    !isCriticalSuccess && finalDieResult <= extremeSuccessLevel;
  const isHardSuccess =
    !isCriticalSuccess &&
    !isExtremeSuccess &&
    finalDieResult <= hardSuccessLevel;
  const isRegularSuccess =
    isSuccess && !isCriticalSuccess && !isExtremeSuccess && !isHardSuccess;
