...
function TorResultsModal() {
  ...
  const successDiceResultsSorted = successDiceResults.sort(
    (a: number, b: number) => a - b
  );
  const specialSuccessesAmount = successDiceResults.filter(
  ...
  return (
    <Modal show={isResultsModalOpen} onHide={closeResultsModal}>
        ...
        <Modal.Body>
          ...
          <section className={torStyles.favouredRollSection}>
            <div className={torStyles.checkboxContainer}>
              <div className={classNames({ [torStyles.dimmed]: !isFavoured })}>
                {isFavoured ? (
                  <FontAwesomeIcon icon={faCheck} />
                ) : (
                  <FontAwesomeIcon icon={faTimes} />
                )}{" "}
                Favoured Roll <FavouredTooltip />
              </div>
              <div
                className={classNames({ [torStyles.dimmed]: !isIllFavoured })}
              >
                {isIllFavoured ? (
                  <FontAwesomeIcon icon={faCheck} />
                ) : (
                ...
                Ill-Favoured Roll <IllFavouredTooltip />
              </div>
            </div>
          </section>
          <section
          ...
          >
            ...
            >
              <div>
                {successDiceResults && joinAsBlocks(successDiceResultsSorted)}
              </div>
            </div>
          </section>
          ...
          >
            ...
            <div
              className={classNames({
                ...
                [torStyles.isFirstScoreDimmed]:
                  featDiceResults.length > 1 &&
                  (() => {
          ...
          {Number(modifier) !== 0 && !isNaN(Number(modifier)) && (
            <section
              className={classNames(
                styles.poolResultsBlock,
                styles.resultsBlock,
                torStyles.resultsModifierBlock
              )}
            >
              <span>
                <FontAwesomeIcon icon={faPlus} />
              </span>{" "}
              <span className={torStyles.modifierResult}>{modifier}</span>
              (Modifier)
            </section>
          )}
          ...
          >
            <span className={torStyles.equalsContainer}>
              <FontAwesomeIcon
                className={torStyles.equalsIcon}
                icon={faEquals}
              />
            </span>
            <span
            ...
            {isAutoSuccess ? (
              <span>
                ...
                {isAdversary ? (
                  <FontAwesomeIcon
                    className={torStyles.resultIcon}
                    icon={faEye}
                  />
                ) : (
                  <img
                    className={torStyles.resultImg}
                    src={require("./assets/gandalf-rune-success.png")}
                    alt="Gandalf Icon"
                  />
                )}{" "}
              </span>
            ...
            {isAutoFailure ? (
              <span>
                ...
                {isAdversary ? (
                  <img
                    className={torStyles.resultImg}
                    src={require("./assets/gandalf-rune-failure.png")}
                    alt="Gandalf Icon"
                  />
                ) : (
                  <FontAwesomeIcon
          ...
          {isSuccess && specialSuccessesAmount > 0 && (
            <section className={torStyles.specialSuccessesSection}>
              <div>
                <span className={torStyles.specialSuccessesIcon}>
                  <ElvenRune />
                </span>{" "}
                <CodeSpan>{specialSuccessesAmount}</CodeSpan> Special{" "}
                {specialSuccessesAmount === 1 ? "Success" : "Successes"}
              </div>
            </section>
          )}
          <section>
            ...
            <div className={torStyles.checkboxContainer}>
              <div className={classNames({ [torStyles.dimmed]: !isWeary })}>
                {isWeary ? (
                  <FontAwesomeIcon icon={faCheck} />
                ) : (
                  <FontAwesomeIcon icon={faTimes} />
              ...
              <div className={classNames({ [torStyles.dimmed]: !isMiserable })}>
                {isMiserable ? (
                  <FontAwesomeIcon icon={faCheck} />
                ) : (
                  <FontAwesomeIcon icon={faTimes} />
                )}{" "}
                Miserable <MiserableTooltip />
              </div>
            </div>
          </section>
          <section>
            ...
            <div className={torStyles.checkboxContainer}>
              <div className={classNames({ [torStyles.dimmed]: !isAdversary })}>
                {isAdversary ? (
                  <FontAwesomeIcon icon={faCheck} />
                ) : (
                  <FontAwesomeIcon icon={faTimes} />
                )}{" "}
                Rolling for an Adversary <AdversaryRollTooltip />
              </div>
            </div>
          </section>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="outline-primary" onClick={onReroll}>
            Reroll
          </Button>
          <Button variant="outline-secondary" onClick={closeResultsModal}>
            Close
          </Button>
        </Modal.Footer>
      </div>
