...
function WarhammerMoneyModal({
  ...
  const [moneyState, setMoneyState] =
    useState<MoneyStateTypes>(initialMoneyState);
  const [moneyToAddState, setMoneyToAddState] =
    useState<MoneyStateTypes>(initialMoneyState);
  const [moneyResultState, setMoneyResultState] =
    useState<MoneyStateTypes>(initialMoneyState);
  ...
  const changeToPennies = ({
    MONEY_GOLD,
    MONEY_SILVER,
    MONEY_BRASS,
  }: MoneyStateTypes): number => {
    return (
      Number(MONEY_GOLD) * 240 + Number(MONEY_SILVER) * 12 + Number(MONEY_BRASS)
    );
  };
  ...
  const handleRecalculate = () => {
    ...
    if (operationState === "ADD") {
      penniesResult =
        changeToPennies(moneyState) + changeToPennies(moneyToAddState);
    } else {
      penniesResult =
        changeToPennies(moneyState) - changeToPennies(moneyToAddState);
    }
  ...
  const tooltipContent = (
    <>
      <p>
        <span className={styles.silver}>1 silver shilling (1 ss)</span> ={" "}
        <span className={styles.brass}>12 brass pennies (12 d)</span>
      </p>
      <p>
        <span className={styles.gold}>1 gold crown (1 GC)</span> ={" "}
        <span className={styles.silver}>20 ss</span> ={" "}
        <span className={styles.brass}>240 d</span>
      </p>
    </>
  );
    ...
    <Modal show={showModal} onHide={closeModal} size="lg">
      ...
      <Modal.Body>
        <div className={styles.container}>
          ...
          <div
            className={classNames([styles.rowToRight, styles.recalculateRow])}
          >
            <div className={styles.buttonsContainer}>
              <Button variant="outline-warning" onClick={handleClearMoneyState}>
          ...
          <div className={styles.rowToRight}>
            <TooltipWrapper
              content={!hasHookUrl ? "No Discord Webhook url provided" : ""}
            >
              <div
                className={classNames({
              ...
              >
                <Form.Group
                  ...
                  controlId="sendToDiscordCheckbox"
                >
                  <Form.Check
