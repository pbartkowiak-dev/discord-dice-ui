...
export const getDiscordMsgData = ({
  ...
  const msgTitle = `${username} ${
    wasRerolled ? "__rerolled__" : "rolled"
  } \`${finalScore}\` against \`${tn}\` TN`;
  ...
  description += "\n";
  description += boldWrap(`${isWeary ? YES_ICON : NO_ICON} Weary`, isWeary);
  ...
  fields.push({
    name: `:game_die: ${
      successDiceResults.length === 1
        ? "Success Die Roll Result"
        : "Success Dice Roll Results"
    }:`,
  ...
  if (modifier && !isNaN(modifierNumber) && modifierNumber !== 0) {
    ...
    const modifierIcon =
      modifierNumber > 0 ? ":heavy_plus_sign:" : ":heavy_minus_sign:";
  ...
  fields.push({
    name: `:zap: ${
      featDiceResults.length === 1
        ? "Feat Die Roll Result"
        : "Feat Dice Roll Results"
    }:`,
