...
const getNewResult = ({
  ...
  return {
    ...
    style: {
      transform: `rotate(${getRandom(
        90,
        -90
      )}deg) scale(.95) translate(${getRandom(5, -5)}px, ${getRandom(
        5,
        -5
      )}px)`,
    },
...
const useStore = create<State>((set, get) => ({
  ...
  rollDice: (pool, isRerollingAllDice) => {
    ...
    if (skillDice) {
      ...
      const normalIcons = results.filter(
        (val) => val === 4 || val === 5
      ).length;
      ...
      const resultsMapped = results.map((val, index) =>
        getNewResult({
          val,
          id: index,
  ...
  rerollSelected: () => {
    ...
    const normalIcons = rerolledResults.filter(
      ({ val }) => val === 4 || val === 5
    ).length;
    const exaltedIcons = rerolledResults.filter(({ val }) => val === 6).length;
    const totalIcons = normalIcons + exaltedIcons * 2;
    const wrathDieResults = rerolledResults
  ...
  increaseDicePool: (amount) => {
    ...
    const newNormalIcons = addedDiceResults.filter(
      (val) => val === 4 || val === 5
    ).length;
    ...
    const newResults = addedDiceResults.map((val, index) =>
      getNewResult({
        val,
        id: index + 500,
