...
const WrathAndGloryResultsModal: FC = () => {
  ...
  const wasAllDiceRerolled: number[] = useWrathAndGloryStore(
    ({ wasAllDiceRerolled }) => wasAllDiceRerolled
  );
  const isModalOpen = useWrathAndGloryStore(({ isModalOpen }) => isModalOpen);
  const closeModal: () => void = useWrathAndGloryStore(
    ({ closeModal }) => closeModal
  );
  const selectedIds: number[] = useWrathAndGloryStore(
    ({ selectedIds }) => selectedIds
  );
  const rerollAll: number[] = useWrathAndGloryStore(
    ({ rerollAll }) => rerollAll
  );
  const rerollSelected: number[] = useWrathAndGloryStore(
    ({ rerollSelected }) => rerollSelected
  );
  const increaseDicePool: number[] = useWrathAndGloryStore(
    ({ increaseDicePool }) => increaseDicePool
  );
  const areDiceAdded: number[] = useWrathAndGloryStore(
    ({ areDiceAdded }) => areDiceAdded
  );
      ...
      <div className={styles.modalWrapper}>
        ...
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            {wasAllDiceRerolled ? <small>(all dice rerolled)</small> : null}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
