...
import {
  closeWarhammerResultsModal,
  requestWarhammerReroll,
...
function WarhammerResultsModal() {
  ...
  const warhammerState = useSelector(
  ...
  const { isSuccess, isAutoSuccess, isFailure, isAutoFailure, isDouble } =
    successLevels;
  ...
  let slString =
    successLevels.SL > 0 ? `+${successLevels.SL}` : `${successLevels.SL}`;
  ...
  return (
    <Modal show={showResultsModal} onHide={hideModal}>
      ...
      <Modal.Body>
        ...
        {/* Roll Data */}
        {isSuccess && !isAutoSuccess && (
          <div
            className={classNames([
              styles.generalResult,
              styles.generalResultSuccess,
            ])}
          >
            Success
          </div>
        ...
        {isFailure && !isAutoFailure && (
          <div
            className={classNames([
              styles.generalResult,
              styles.generalResultFailure,
            ])}
          >
            Failure
          </div>
        ...
        {isDouble && (
          <div className={classNames([styles.generalResult])}>Double</div>
        )}
        {rerollCount > 0 && (
        ...
        <div className={styles.slResult}>
          <div>
            <span className={styles.slResultLabel}>{slWord}:</span>
          </div>
          <div>
            <CodeSpan className={styles.slResultSpan}>{slString}</CodeSpan>
          </div>
        </div>
        ...
        <div className={rerollStyles.container}>
          <div className={rerollStyles.row}>
            <Button variant="outline-primary" onClick={handleReroll}>
              Reroll
            </Button>
          </div>
        </div>
