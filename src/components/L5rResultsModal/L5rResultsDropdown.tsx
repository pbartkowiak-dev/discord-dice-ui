  ...
  return (
    <Dropdown className="dropdown-wrapper">
      ...
      <Dropdown.Menu>
        ...
        {isModifyingAllowed && (
          <Dropdown.Item
            onClick={() => handleAlterDie(`${OPPORTUNITY}_${STRIFE}`)}
          >
            Set to {OpportunityImg} {StrifeImg}
          </Dropdown.Item>
        )}
        {isModifyingAllowed && (
          <Dropdown.Item onClick={() => handleAlterDie(`${OPPORTUNITY}`)}>
            Set to {OpportunityImg}
          </Dropdown.Item>
        )}
        {isModifyingAllowed && (
          <Dropdown.Item onClick={() => handleAlterDie(`${SUCCESS}_${STRIFE}`)}>
            Set to {SuccessImg} {StrifeImg}
          </Dropdown.Item>
        )}
        {isModifyingAllowed && (
          <Dropdown.Item onClick={() => handleAlterDie(`${SUCCESS}`)}>
            Set to {SuccessImg}
          </Dropdown.Item>
        )}
        {isModifyingAllowed && (
          <Dropdown.Item
            onClick={() => handleAlterDie(`${EXPLOSIVE_SUCCESS}_${STRIFE}`)}
          >
