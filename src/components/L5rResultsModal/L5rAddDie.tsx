...
function L5rResultsDropdown({ type, l5rAddDie }: any) {
  ...
  return (
    <Dropdown className="dropdown-wrapper">
          ...
          <div className={l5rStyles.addDieContainer}>
            <div className={l5rStyles.addDie}>
              <div className={l5rStyles.addDieInner}>
                <FontAwesomeIcon
                  className={l5rStyles.addDieIcon}
                  icon={faPlus}
                />
              </div>
            </div>
      ...
      <Dropdown.Menu>
        <Dropdown.Item onClick={() => handleAddDie()}>
          Roll a new {BlackRingImg} die
        </Dropdown.Item>
        <Dropdown.Divider />
        <Dropdown.Item onClick={() => handleAddDie(`${OPPORTUNITY}_${STRIFE}`)}>
          Add a {dieTypeString} {BlackRingImg} set to {OpportunityImg}{" "}
          {StrifeImg}
        </Dropdown.Item>
        <Dropdown.Item onClick={() => handleAddDie(`${OPPORTUNITY}`)}>
          Add a {dieTypeString} {BlackRingImg} set to {OpportunityImg}
        </Dropdown.Item>
        <Dropdown.Item onClick={() => handleAddDie(`${SUCCESS}_${STRIFE}`)}>
          Add a {dieTypeString} {BlackRingImg} set to {SuccessImg} {StrifeImg}
        </Dropdown.Item>
        <Dropdown.Item onClick={() => handleAddDie(`${SUCCESS}`)}>
          Add a {dieTypeString} {BlackRingImg} set to {SuccessImg}
        </Dropdown.Item>
        <Dropdown.Item
          onClick={() => handleAddDie(`${EXPLOSIVE_SUCCESS}_${STRIFE}`)}
        >
          Add a {dieTypeString} {BlackRingImg} set to {ExplosiveSuccessImg}{" "}
          {StrifeImg}
        </Dropdown.Item>
      </Dropdown.Menu>
    </Dropdown>
