...
function L5rResultsModal({
  ...
  const selectDie = (index: number) => {
    setSelectedDiceState([...selectedDiceState, index]);
  };

  const deselectDie = (index: number) => {
    setSelectedDiceState(selectedDiceState.filter((i: number) => i !== index));
  };
  ...
  results.forEach((result: string | number, index: number) => {
    ...
    const handleClick = isDiceSelected ? deselectDie : selectDie;
  ...
  resultsKept.forEach((result: string, index: number) => {
    ...
    const diceImgClassNames = classNames({
      [l5rStyles.img]: true,
      [l5rStyles.noOptionAvailable]:
        (!isModifyingAllowed && !isExplosiveDie) || wasAlreadyExploded,
  ...
  additionalDiceRolled.forEach((result: string, index: number) => {
    ...
    const diceImgClassNames = classNames({
      [l5rStyles.img]: true,
      [l5rStyles.noOptionAvailable]:
        isAdditionalDieKept && (!isExplosiveDie || wasAlreadyExploded),
    ...
    if (isAdditionalDieKept) {
      content = (
        <L5rResultsDropdownContainer
          ...
          type={ADDITIONAL_DIE}
        >
          <TooltipWrapper content={diceLabel || result}>{img}</TooltipWrapper>
        </L5rResultsDropdownContainer>
      );
    } else {
      content = (
      ...
      >
        ...
        >
          <h4 className={l5rStyles.header}>
            Select the dice to reroll or keep:
          </h4>
          <div className={l5rStyles.resultsContainer}>{resultsElements}</div>
          <div className={l5rStyles.firstButtonsRow}>
            <TooltipWrapper
              content={rerollCount > 0 ? "Dice already rerolled!" : ""}
            >
              <span
                className={classNames({
