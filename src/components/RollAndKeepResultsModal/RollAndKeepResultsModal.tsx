...
function RollAndKeepResultsModal({
  ...
  const selectDie = (rowIndex: number) => {
    setSelectedDiceState([...selectedDiceState, rowIndex]);
  };
  ...
  const getTotal = () => {
    return selectedDiceState.reduce((acc: Number, selectedIndex: number) => {
      return (
        acc + results[selectedIndex].reduce((a: number, b: number) => a + b, 0)
      );
    }, 0);
  };

  const resultsElements: Array<JSX.Element> = results.map(
    (resultsInner: number[], rowIndex: number) => {
      ...
      return (
        ...
        >
          <div className={rollAndKeepStyles.sum}>{sum}</div>
          {resultsInner.map((result: number, index: number) => {
            ...
            return (
              <div
                key={`row-index-${index}`}
                className={rollAndKeepStyles.resultsBlock}
              >
                <div className={rollAndKeepStyles.imgContainer}>
                  <img
                    ...
                    alt={"diceLabel"}
                  />
                  <span className={rollAndKeepStyles.imgResult}>
                    {showResult}
                  </span>
                </div>
                <div
                ...
                >
                  <FontAwesomeIcon
                    className={rollAndKeepStyles.arrowIcon}
                    icon={faArrowRight}
                  />
                </div>
              </div>
  ...
  if (modifier !== 0) {
    ...
    modifierElem = (
      <div>
        (with <CodeSpan>{modifierStr}</CodeSpan> modifier)
      </div>
    );
  }
    ...
    <Modal show={showModal} onHide={hideMsg}>
      ...
      <Modal.Body className={styles.resultsBody}>
        <section>
          ...
          <div className={rollAndKeepStyles.results}>{resultsElements}</div>
          <div className={rollAndKeepStyles.equalsRow}>
            <div className={rollAndKeepStyles.equalsContainer}>
              <FontAwesomeIcon
                className={rollAndKeepStyles.equalsIcon}
                icon={faEquals}
              />
            </div>
            <div className={rollAndKeepStyles.total}>{total}</div>{" "}
            {modifierElem}
          </div>
        </section>
