...
import useCombatTrackerStore, {
  CombatantTypes,
  CombatantTypesEditableFields,
} from "./store";
...
export default function Combatant({
  ...
  const setIsDragging = useCombatTrackerStore(
    ({ setIsDragging }) => setIsDragging
  );
  const setCombatantZone = useCombatTrackerStore(
    ({ setCombatantZone }) => setCombatantZone
  );
  const forceUpdateCombatants = useCombatTrackerStore(
    ({ forceUpdateCombatants }) => forceUpdateCombatants
  );
  const deleteCombatant = useCombatTrackerStore(
    ({ deleteCombatant }) => deleteCombatant
  );
  const cloneCombatant = useCombatTrackerStore(
    ({ cloneCombatant }) => cloneCombatant
  );
  const updateCombatant = useCombatTrackerStore(
    ({ updateCombatant }) => updateCombatant
  );
  const lockCombatant = useCombatTrackerStore(
    ({ lockCombatant }) => lockCombatant
  );
  ...
  return (
    <Draggable handle=".handle" onStart={handleStart} onStop={handleStop}>
