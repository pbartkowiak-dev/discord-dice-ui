...
function CombatTrackerModal() {
  ...
  const closeModal: () => void = useCombatTrackerStore(
    ({ closeModal }) => closeModal
  );
  const clearState: () => void = useCombatTrackerStore(
  ...
  return (
    <Modal show={showModal} onHide={closeModal} size="lg">
      ...
      <Modal.Body>
        <Zone name="Waiting Room" index={-1} key={`waitingRoom:${renders}`} />
        <AddZone />
        <section>
          {zones.map((zone: string, i: number) => (
            <Zone name={zone} index={i} key={`${i}:${renders}`} />
          ))}
        </section>
      </Modal.Body>
      <Modal.Footer>
