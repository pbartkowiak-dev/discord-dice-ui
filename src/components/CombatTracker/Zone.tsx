...
export default function Zone({ name, index }: ZoneProps) {
  ...
  const combatants = useCombatTrackerStore(({ combatants }) =>
    combatants
  ...
  const setHoverZone = useCombatTrackerStore(
  ...
  return (
    ...
    >
      ...
      <div>
        {combatants.map((combatant: CombatantTypes) => (
          <Combatant key={combatant.id} {...combatant} />
        ))}
      </div>
