...
function CthulhuPushOptions({
	...
	const luckForHardSuccess = Number(finalDieResult) - Math.floor( Number(skillLevel) / 2 );
	const luckForExtremeSuccess = Number(finalDieResult) - Math.floor( Number(skillLevel) / 5 );
	...
	return (
		<div className={styles.container}>
			...
			<div className={classNames([styles.row, styles.luckSpends])}>
				{ !!luckForSuccess && <div>Spend <CodeSpan>{ luckForSuccess }</CodeSpan> Luck Points for <strong>Success</strong></div> }
				{ !!luckForHardSuccess && <div>Spend <CodeSpan>{ luckForHardSuccess }</CodeSpan> Luck Points for <strong>Hard Success</strong></div> }
				{ !!luckForExtremeSuccess && <div>Spend <CodeSpan>{ luckForExtremeSuccess }</CodeSpan> Luck Points for <strong>Extreme Success</strong></div> }
				<InfoTooltip
