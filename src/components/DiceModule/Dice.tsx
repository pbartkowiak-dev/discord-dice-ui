...
function Dice({
  ...
  const DropdownContent = () => {
    const maxDiceInOneRoll =
      diceType === D6_CONAN || diceType === D6_INFINITY ? 15 : 10;
    const options = new Array(maxDiceInOneRoll).fill("").map((_, index) => {
      ...
      return (
        ...
        >
          <span>
            Roll <strong>{index + 1}</strong> {dieWord}
          </span>
        </Dropdown.Item>
      );
    });
  ...
  return (
    <Card className={`dice dice--${diceType}`}>
      ...
      <Card.Footer>
        {noDropdown ? (
          <Button onClick={() => handleRollDice(diceType)} variant="primary">
            {label}
          </Button>
        ) : (
          <DropdownButton
