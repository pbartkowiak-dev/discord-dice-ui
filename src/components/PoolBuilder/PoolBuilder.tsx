...
function PoolBuilder({
  ...
  const PoolBuilderDice = diceSet.map(
    ({ diceType, label, diceImg, isExcludedFromPool, noDropdown }) => {
  ...
  return (
    <form id={formName} className={styles.diceContainer} onSubmit={onSubmit}>
      {PoolBuilderDice}
    </form>
  );
}
