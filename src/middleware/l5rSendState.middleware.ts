...
export default (store: any) => (next: any) => (action: any) => {
	if (action.type === L5R_SEND_STATE) {
		...
		if (!resultsKept.length) {
			const resultsJoined = joinAsBlocks(l5rResultsToLabel(results), ',\n', true);
			fields.push({
		...
		if (resultsKept.length && !additionalDiceKept.length) {
			const resultsKeptJoined = joinAsBlocks(l5rResultsToLabel(resultsKept), ',\n', true);
			fields.push({
		...
		if (additionalDiceKept.length) {
			const additionalDiceKeptJoined = joinAsBlocks(l5rResultsToLabel(additionalDiceKept), ',\n', true);
			fields.push({
