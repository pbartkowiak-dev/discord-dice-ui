...
export default (store: any) => (next: any) => (action: any) => {
  if (action.type === COMBAT_TRACKER_SEND_REQUESTED) {
    const showAdvantage =
      diceModuleOptionsStore.getState().mode === "warhammerMode";

    const combatTrackerState = useCombatTrackerStore.getState();
    const { combatants, zones } = combatTrackerState;

    const showConditions = !!combatants.filter((c) => c.conditions?.length > 0)
      .length;
    ...
    const longestAdvantage = showAdvantage
      ? combatants.reduce(
          (acc, c) => Math.max(acc, ` (${c.advantage})`.length),
          0
        )
      : 0;
    ...
    zones.forEach((zoneName, currentZone) => {
      ...
      zoneCombatants.forEach((c) => {
        ...
        const name = `${c.name}${
          showAdvantage ? ` (${c.advantage})` : ""
        }`.padEnd(longestName + longestAdvantage);
        const hp = `${c.hp}/${c.hpMax}`.padStart(
          showWounds ? woundsPad - 3 : longestHP
        );
        ...
        if (c.conditions || c.wounds) {
          const conditionsChunks = chunkString(
            c.conditions,
            longestName + longestAdvantage
          );
          const woundsChunks = chunkString(c.wounds, woundsPad);

          const additionalLines = Math.max(
            conditionsChunks.length,
            woundsChunks.length
          );
          ...
          for (let line = 0; line < additionalLines; line += 1) {
            ...
            description += `${initiativePlaceholder}${conditions.padEnd(
              longestName + longestAdvantage
            )} | ${wounds.padStart(showWounds ? woundsPad : 0)}\n`;
          }
        }
      });
