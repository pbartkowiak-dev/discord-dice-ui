...
export default (store: any) => (next: any) => (action: any) => {
  if (action.type === WARHAMMER_DICE_ROLLED) {
    ...
    const { SL, isSuccess, isAutoSuccess, isDouble } = successLevels;
    ...
    const successLevelIcon =
      isSuccess || isAutoSuccess ? ":green_circle:" : ":red_circle:";
    const skillLevelString =
      skillLevel <= 9 ? `0${skillLevel}` : `${skillLevel}`;
    ...
    store.dispatch(
      requestMsgReady({
        ...
        description,
        color:
          isSuccess || isAutoSuccess ? getColor(SUCCESS) : getColor(FAILURE),
        fields,
      })
    );
  }
