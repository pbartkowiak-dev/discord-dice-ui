...
const getLocalMsg = (store: any) => (next: any) => (action: any) => {
  if (action.type === DICE_ROLLED) {
    ...
    if (
      ...
      fields.push(<>Modifier: {modifierWithSymbol}.</>);
    }

    if (
      (hasMultipleDice || useModifier || isFate) &&
      !isCombatDie &&
      !(isConanHitLocationDie || isInfinityHitLocationDie)
    ) {
      if (useModifier) {
        fields.push(
          <>
            {IconRight} Total (with {modifierWithSymbol} modifier):{" "}
            <CodeSpan>{fateResultTotal || totalWithModifier}</CodeSpan>.
          </>
        );
      } else {
        fields.push(
          <>
            {IconRight} Total:{" "}
            <CodeSpan>{fateResultTotal || totalWithoutModifier}</CodeSpan>.
          </>
        );
      }
    }
    ...
    if (isCombatDie) {
      fields.push(<strong>Combat Die Results:</strong>);
      fields.push(
        <>
    ...
    if (isFate) {
      ...
      titleMsg = (
        <div>
          <strong>You rolled</strong> <CodeSpan>{rolled}</CodeSpan>
          <strong>. {rolledWord}:</strong>
        </div>
      );
