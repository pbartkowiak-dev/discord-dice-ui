...
export default (store: any) => (next: any) => (action: any) => {
	if (action.type === UPDATE_NARRATIVE_TOKENS_STATE) {
		const { payload: { destinyLight, destinyDark } } = action;
		...
		fields.push({
			...
			value: destinyDarkNum === 0 ? '-' : new Array(destinyDarkNum).fill(':black_circle:').join(' ')
		})

		fields.push({
			name: `Light: \`${destinyLight}\``,
			value: destinyLightNum === 0 ? '-' : new Array(destinyLightNum).fill(':white_circle:').join(' ')
		})
