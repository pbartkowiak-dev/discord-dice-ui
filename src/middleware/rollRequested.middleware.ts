...
import { DICE_ROLL_REQUESTED, diceRolled } from "../actions/roll.actions";
...
export default (store: any) => (next: any) => (action: any) => {
  if (action.type === DICE_ROLL_REQUESTED) {
    ...
    result.totalWithModifier = result.results.reduce(
    ...
    if (diceType === D6_CONAN) {
      const combatDieResults = result.results.reduce(
        (total, current) => {
          if (current >= 5) {
    ...
    if (diceType === FATE_DIE) {
      ...
      const fateResultTotal =
        result.fateResults.reduce((resultTotal, current) => {
          if (current === PLUS) {
